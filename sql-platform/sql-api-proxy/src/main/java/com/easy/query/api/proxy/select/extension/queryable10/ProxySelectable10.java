...
public interface ProxySelectable10<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default  <TRProxy extends ProxyEntity<TRProxy, TR>, TR> ProxyQueryable<TRProxy, TR> select(TRProxy trProxy, SQLSelectAs... sqlSelectAs){
        ...
        ClientQueryable<TR> select = getClientQueryable10().select(trProxy.getEntityClass(), columnAsSelector -> {
    ...
    default <TRProxy extends ProxyEntity<TRProxy, TR>, TR> ProxyQueryable<TRProxy, TR> select(TRProxy trProxy, SQLExpression1<MultiProxyAsSelector10<T1Proxy, T2Proxy, T3Proxy, T4Proxy, T5Proxy, T6Proxy, T7Proxy, T8Proxy, T9Proxy, T10Proxy,TRProxy, TR>> selectExpression) {
