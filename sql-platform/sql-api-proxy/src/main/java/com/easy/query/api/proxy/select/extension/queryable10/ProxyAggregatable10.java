...
public interface ProxyAggregatable10<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default <TMember extends Number> BigDecimal sumBigDecimalOrDefault(SQLColumn<?,TMember> sqlColumn, BigDecimal def) {
        ...
        List<TMember> result = getClientQueryable10().selectAggregateList(sqlColumn.getTable(), sumFunction, sqlColumn.getValue(), null);
    ...
    default <TMember> TMember maxOrDefault(SQLColumn<?,TMember> sqlColumn, TMember def) {
        ...
        List<TMember> result = getClientQueryable10().selectAggregateList(sqlColumn.getTable(), maxFunction, sqlColumn.getValue(), null);
        return EasyCollectionUtil.firstOrDefault(result, def);
    }
    ...
    default <TMember> TMember minOrDefault(SQLColumn<?,TMember> sqlColumn, TMember def) {
        ...
        List<TMember> result = getClientQueryable10().selectAggregateList(sqlColumn.getTable(), minFunction, sqlColumn.getValue(), null);
        return EasyCollectionUtil.firstOrDefault(result, def);
    }
    ...
    default <TMember extends Number, TResult extends Number> TResult avgOrDefault(SQLColumn<?,TMember> sqlColumn, TResult def, Class<TResult> resultClass) {
        ...
        List<TResult> result = getClientQueryable10().selectAggregateList(sqlColumn.getTable(), avgFunction, sqlColumn.getValue(), resultClass);
        return EasyCollectionUtil.firstOrDefault(result, def);
    }
