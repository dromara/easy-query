...
public interface ProxyAggregatable5<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default <TMember extends Number> BigDecimal sumBigDecimalOrDefault(SQLColumn<?,TMember> sqlColumn, BigDecimal def) {
        ...
        List<TMember> result = getClientQueryable5().selectAggregateList(sqlColumn.getTable(), sumFunction, sqlColumn.value(), null);
    ...
    default <TMember> TMember maxOrDefault(SQLColumn<?,TMember> sqlColumn, TMember def) {
        ...
        List<TMember> result = getClientQueryable5().selectAggregateList(sqlColumn.getTable(), maxFunction, sqlColumn.value(), null);
        return EasyCollectionUtil.firstOrDefault(result, def);
    }
    ...
    default <TMember> TMember minOrDefault(SQLColumn<?,TMember> sqlColumn, TMember def) {
        ...
        List<TMember> result = getClientQueryable5().selectAggregateList(sqlColumn.getTable(), minFunction, sqlColumn.value(), null);
        return EasyCollectionUtil.firstOrDefault(result, def);
    }
    ...
    default <TMember extends Number, TResult extends Number> TResult avgOrDefault(SQLColumn<?,TMember> sqlColumn, TResult def, Class<TResult> resultClass) {
        ...
        List<TResult> result = getClientQueryable5().selectAggregateList(sqlColumn.getTable(), avgFunction, sqlColumn.value(), resultClass);
        return EasyCollectionUtil.firstOrDefault(result, def);
    }
