...
import com.easy.query.api.proxy.select.extension.queryable9.sql.MultiProxySelector9;
import com.easy.query.api.proxy.select.extension.queryable9.sql.impl.MultiProxyAsSelector9Impl;
import com.easy.query.api.proxy.select.extension.queryable9.sql.impl.MultiProxySelector9Impl;
...
public interface ProxySelectable9<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default ProxyQueryable<T1Proxy, T1> select(SQLSelect... sqlSelects){
        ...
        ClientQueryable<T1> select = getClientQueryable9().select(columnSelector -> {
    ...
    default  <TRProxy extends ProxyEntity<TRProxy, TR>, TR> ProxyQueryable<TRProxy, TR> select(TRProxy trProxy, SQLSelectAs... sqlSelectAs){
        ...
        ClientQueryable<TR> select = getClientQueryable9().select(trProxy.getEntityClass(), columnAsSelector -> {
    ...
    default ProxyQueryable<T1Proxy, T1> select(SQLExpression1<MultiProxySelector9<T1Proxy,T2Proxy,T3Proxy,T4Proxy,T5Proxy,T6Proxy,T7Proxy,T8Proxy,T9Proxy>> selectExpression){

        ClientQueryable<T1> select = getClientQueryable9().select(columnSelector -> {
            selectExpression.apply(new MultiProxySelector9Impl<>(columnSelector.getSelector(), get1Proxy(),get2Proxy(),get3Proxy(),get4Proxy(),get5Proxy(),get6Proxy(),get7Proxy(),get8Proxy(),get9Proxy()));
        });
        return new EasyProxyQueryable<>(get1Proxy(), select);
    }
