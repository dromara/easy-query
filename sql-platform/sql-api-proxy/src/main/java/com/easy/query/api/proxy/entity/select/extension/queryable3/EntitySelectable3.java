...
public interface EntitySelectable3<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default <TRProxy extends ProxyEntity<TRProxy, TR>, TR extends ProxyEntityAvailable<TR, TRProxy>> EntityQueryable<TRProxy, TR> select(Class<TR> resultClass, SQLFuncExpression4<T1Proxy, T2Proxy, T3Proxy, TRProxy, SQLSelectAs> selectExpression) {
        ...
        ClientQueryable<TR> select = getClientQueryable3().select(resultClass, (selector1, selector2, selector3) -> {
            SQLSelectAs sqlSelectAs = selectExpression.apply(get1Proxy(), get2Proxy(), get3Proxy(), trProxy);
            sqlSelectAs.accept(selector1.getAsSelector());
        });
        return new EasyEntityQueryable<>(trProxy, select);
    }

    default <TRProxy extends ProxyEntity<TRProxy, TR>, TR extends ProxyEntityAvailable<TR, TRProxy>> EntityQueryable<TRProxy, TR> selectMerge(Class<TR> resultClass, SQLFuncExpression1<Tuple4<T1Proxy, T2Proxy, T3Proxy, TRProxy>, SQLSelectAs> selectExpression) {
