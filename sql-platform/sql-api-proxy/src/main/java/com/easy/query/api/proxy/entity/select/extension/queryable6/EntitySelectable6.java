...
public interface EntitySelectable6<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default <TRProxy extends ProxyEntity<TRProxy, TR>, TR> EntityQueryable<TRProxy, TR> select(SQLFuncExpression6<T1Proxy, T2Proxy, T3Proxy, T4Proxy, T5Proxy, T6Proxy, TRProxy> selectExpression) {
        ...
        if (resultProxy instanceof EntityFetcher) {
            ...
            return Select.selectProxy(EasyObjectUtil.typeCastNullable(resultProxy1.fetchProxy()), getClientQueryable6());
        }
        EntityQueryable<TRProxy, TR> trProxyTREntityQueryable = Select.selectProxy(resultProxy, getClientQueryable6());
    ...
    default <TR> Query<TR> select(Class<TR> resultClass, SQLFuncExpression6<T1Proxy, T2Proxy,T3Proxy,T4Proxy,T5Proxy,T6Proxy, SQLSelectAsExpression> selectExpression) {
        ...
        return getClientQueryable6().select(resultClass, columnAsSelector -> {
    ...
    default <TR> Query<TR> selectAutoInclude(Class<TR> resultClass, SQLFuncExpression6<T1Proxy, T2Proxy, T3Proxy, T4Proxy,T5Proxy,T6Proxy, SQLSelectAsExpression> selectExpression) {
        return selectAutoInclude(resultClass, selectExpression, false);
    }

    default <TR> Query<TR> selectAutoInclude(Class<TR> resultClass, SQLFuncExpression6<T1Proxy, T2Proxy, T3Proxy, T4Proxy,T5Proxy,T6Proxy, SQLSelectAsExpression> selectExpression, boolean replace) {
        SQLSelectAsExpression sqlSelectAsExpression = selectExpression.apply(get1Proxy(), get2Proxy(), get3Proxy(), get4Proxy(),get5Proxy(),get6Proxy());
        return getClientQueryable6().selectAutoInclude(resultClass, (columnAsSelector1, columnAsSelector2, columnAsSelector3, columnAsSelector4, columnAsSelector5, columnAsSelector6) -> {
