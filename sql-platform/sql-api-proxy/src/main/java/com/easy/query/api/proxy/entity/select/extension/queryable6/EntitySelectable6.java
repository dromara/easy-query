...
public interface EntitySelectable6<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default <TRProxy extends ProxyEntity<TRProxy, TR>, TR extends ProxyEntityAvailable<TR, TRProxy> & Draft> EntityQueryable<TRProxy, TR> selectDraft(SQLFuncExpression6<T1Proxy,T2Proxy,T3Proxy,T4Proxy,T5Proxy,T6Proxy, DraftFetcher<TR,TRProxy>> selectExpression){
    ...
    default <TRProxy extends ProxyEntity<TRProxy, TR>, TR extends ProxyEntityAvailable<TR, TRProxy> & Draft> EntityQueryable<TRProxy, TR> selectDraftMerge(SQLFuncExpression1<MergeTuple6<T1Proxy, T2Proxy, T3Proxy, T4Proxy, T5Proxy, T6Proxy>, DraftFetcher<TR,TRProxy>> selectExpression){
    ...
    default <TRProxy extends ProxyEntity<TRProxy, TR>, TR> EntityQueryable<TRProxy, TR>  select(SQLFuncExpression6<T1Proxy,T2Proxy,T3Proxy, T4Proxy,T5Proxy,T6Proxy, TRProxy> selectExpression){
        ...
        TRProxy apply = selectExpression.apply(get1Proxy(),get2Proxy(),get3Proxy(),get4Proxy(),get5Proxy(),get6Proxy());
        SQLSelectAsExpression selectAsExpression = apply.getEntitySQLContext().getSelectAsExpression();
        if(selectAsExpression==null){//全属性映射
            ClientQueryable<TR> select = getClientQueryable6().select(apply.getEntityClass());
            return new EasyEntityQueryable<>(apply, select);
        }else{
            ClientQueryable<TR> select = getClientQueryable6().select(apply.getEntityClass(), columnAsSelector -> {
