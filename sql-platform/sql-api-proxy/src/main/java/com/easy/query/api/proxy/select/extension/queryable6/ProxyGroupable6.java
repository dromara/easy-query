...
public interface ProxyGroupable6<T1Proxy extends ProxyEntity<T1Proxy, T1>, T1,
    ...
    default ProxyQueryable6<T1Proxy, T1, T2Proxy, T2, T3Proxy, T3, T4Proxy, T4, T5Proxy, T5, T6Proxy, T6> groupBy(SQLGroupSelect... propColumns) {
        return groupBy(true, propColumns);
    }
    default ProxyQueryable6<T1Proxy, T1, T2Proxy, T2, T3Proxy, T3, T4Proxy, T4, T5Proxy, T5, T6Proxy, T6> groupBy(boolean condition, SQLGroupSelect... propColumns) {
        if(condition){
            if(EasyArrayUtil.isNotEmpty(propColumns)){
                for (SQLGroupSelect propColumn : propColumns) {
                    getClientQueryable6().groupBy(groupBySelector -> {
