...
public class EasyQueryProperties {
    ...
    /**
     * 仅分片时有效默认0如果需要建议大于 {@link EasyQueryProperties#maxShardingQueryLimit} * 分库数目
     * 执行线程数 如果为0那么采用无界线程池{@link Executors#newCachedThreadPool},如果是大于0采用长度为{@link EasyQueryProperties#executorQueueSize}的有界队列
     * 核心线程数采用{@link EasyQueryProperties#executorCorePoolSize}并且需要比 {@link EasyQueryProperties#executorCorePoolSize}值大
     */
    private int executorMaximumPoolSize = 0;
    /**
     * 当且仅当{@link EasyQueryProperties#executorMaximumPoolSize}>0生效
     */
