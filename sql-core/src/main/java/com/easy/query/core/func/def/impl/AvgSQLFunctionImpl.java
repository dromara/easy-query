 ...
 * @author xuejiaming
 */
public class AvgSQLFunctionImpl extends AbstractExpressionSQLFunction implements DistinctDefaultSQLFunction {
    private final List<ColumnExpression> columnExpressions;
    private boolean distinct = false;
    private boolean hasDefaultValue;

    public AvgSQLFunctionImpl(List<ColumnExpression> columnExpressions) {
    ...
    public String sqlSegment(TableAvailable defaultTable) {
        ...
        if (hasDefaultValue) {
            if (distinct) {
                return String.format("AVG(DISTINCT IFNULL(%s))", String.join(",", params));
            }
            return String.format("AVG(IFNULL(%s))", String.join(",", params));
        } else {
            if (distinct) {
