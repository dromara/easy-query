 ...
 * @author xuejiaming
 */
public class JSONArrayExtractByIndexSQLFunction extends AbstractExpressionSQLFunction {

    private final List<ColumnExpression> columnExpressions;

    public JSONArrayExtractByIndexSQLFunction(List<ColumnExpression> concatExpressions) {
        if (EasyCollectionUtil.isEmpty(concatExpressions)) {
            throw new IllegalArgumentException("JSONArrayExtractByIndexSQLFunction columns empty");
        }
    ...
    public String sqlSegment(TableAvailable defaultTable) {
        if(columnExpressions.size()!=2){
            throw new IllegalArgumentException("JSONArrayExtractByIndexSQLFunction columns size != 2");
        }
