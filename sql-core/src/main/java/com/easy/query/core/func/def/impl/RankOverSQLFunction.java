 ...
 * @author xuejiaming
 */
public class RankOverSQLFunction extends AbstractExpressionSQLFunction implements PartitionBySQLFunction {
    private final List<ColumnExpression> columnExpressions;
    private final int partitionByColumnSize;

    public RankOverSQLFunction(List<ColumnExpression> columnExpressions) {
        if(EasyCollectionUtil.isEmpty(columnExpressions)){
            throw new EasyQueryInvalidOperationException("rank partition by empty");
        }
    ...
    public String sqlSegment(TableAvailable defaultTable) {
        ...
        sql.append("(RANK() OVER (PARTITION BY ");
