...
public class DefaultSqlPredicate<T1> implements SqlWherePredicate<T1> {
    ...
    public <T2> SqlWherePredicate<T1> exists(boolean condition,Queryable<T2> subQueryable, SqlExpression<SqlWherePredicate<T2>> whereExpression) {
        if (condition) {
            ...
            nextPredicateSegment.setPredicate(new ColumnExistsSubQueryPredicate(getTable(), existsQueryable, SqlKeywordEnum.EXISTS, entityExpressionBuilder.getRuntimeContext()));
            nextAnd();
        }
        return this;
    }
    ...
    public <T2> SqlWherePredicate<T1> notExists(boolean condition, Queryable<T2> subQueryable, SqlExpression<SqlWherePredicate<T2>> whereExpression) {
        if (condition) {
            ...
            nextPredicateSegment.setPredicate(new ColumnExistsSubQueryPredicate(getTable(), existsQueryable, SqlKeywordEnum.NOT_EXISTS, entityExpressionBuilder.getRuntimeContext()));
            nextAnd();
        }
        return this;
    }
    ...
    @Override
    public SqlWherePredicate<T1> columnFunc(boolean condition, Property<T1, ?> column, EasyFunc easyFunc, SqlPredicateCompareEnum sqlPredicateCompare, Object val) {
