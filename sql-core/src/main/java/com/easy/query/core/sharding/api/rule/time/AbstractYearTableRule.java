 ...
 * @author xuejiaming
 */
public abstract class AbstractYearTableRule<TEntity> extends AbstractTableRouteRule<TEntity> {
    /**
    ...
    protected RouteFunction<ActualTable> getRouteFilter(TableAvailable table, Object shardingValue, ShardingOperatorEnum shardingOperator, boolean withEntity) {
        ...
        switch (shardingOperator) {
            ...
            case LESS_THAN: {
                ...
                LocalDateTime timeYearFirstDay = EasyUtil.getYearStart(shardingTime);
                if (shardingTime.isEqual(timeYearFirstDay)) {
                    return t -> tableName.compareToIgnoreCase(t.getActualTableName()) > 0;
                }
                return t -> tableName.compareToIgnoreCase(t.getActualTableName()) >= 0;
            }
