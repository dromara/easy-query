 ...
 * @author xuejiaming
 */
public class DuckDBJSONArrayByIndexSQLFunction extends AbstractExpressionSQLFunction {

    private final List<ColumnExpression> columnExpressions;

    public DuckDBJSONArrayByIndexSQLFunction(List<ColumnExpression> concatExpressions) {
        if (EasyCollectionUtil.isEmpty(concatExpressions)) {
            throw new IllegalArgumentException("DuckDBJSONArrayByIndexSQLFunction columns empty");
        }
    ...
    public String sqlSegment(TableAvailable defaultTable) {
        if(columnExpressions.size()!=2){
            throw new IllegalArgumentException("DuckDBJSONArrayByIndexSQLFunction columns size != 2");
        }
