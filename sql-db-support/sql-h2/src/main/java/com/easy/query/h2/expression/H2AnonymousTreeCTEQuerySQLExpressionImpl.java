 ...
 * @author xuejiaming
 */
public class H2AnonymousTreeCTEQuerySQLExpressionImpl implements AnonymousEntityQuerySQLExpression {
    ...
    public H2AnonymousTreeCTEQuerySQLExpressionImpl(String cteTableName,List<String> columnNames, EntitySQLExpressionMetadata entitySQLExpressionMetadata, EntityQuerySQLExpression querySQLExpression){
    ...
    public String toSQL(ToSQLContext toSQLContext) {
        ...
        StringBuilder declareColumnStringBuilder = new StringBuilder();
        declareColumnStringBuilder.append(EasySQLExpressionUtil.getQuoteName(getRuntimeContext(), firstColumn));
        while(iterator.hasNext()){
            String next = iterator.next();
            declareColumnStringBuilder.append(",").append(EasySQLExpressionUtil.getQuoteName(getRuntimeContext(), next));
        }


        StringBuilder sql = new StringBuilder().append(quoteCteTableName).append("(").append(declareColumnStringBuilder).append(")").append(" AS (");
...
//    }
    ...
    public EntityQuerySQLExpression cloneSQLExpression() {
        return new H2AnonymousTreeCTEQuerySQLExpressionImpl(this.cteTableName,new ArrayList<>(this.columnNames),entitySQLExpressionMetadata,querySQLExpression.cloneSQLExpression());
    }
}
