 ...
 * @author xuejiaming
 */
public class DamengJSONArrayByIndexSQLFunction extends AbstractExpressionSQLFunction {

    private final List<ColumnExpression> columnExpressions;

    public DamengJSONArrayByIndexSQLFunction(List<ColumnExpression> concatExpressions) {
        if (EasyCollectionUtil.isEmpty(concatExpressions)) {
            throw new IllegalArgumentException("DamengJSONArrayByIndexSQLFunction columns empty");
        }
    ...
    public String sqlSegment(TableAvailable defaultTable) {
        if(columnExpressions.size()!=2){
            throw new IllegalArgumentException("DamengJSONArrayByIndexSQLFunction columns size != 2");
        }
