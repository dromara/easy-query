...
//            return format.replace("%_a1", "MM").replace("%_a2", "DD").replace("%_a3", "HH24").replace("%_a4", "MI").replace("%_a5", "AM");
    ...
    protected String getReplacedFormats(String format) {
        String pattern = "(yyyy|yy|MM|dd|HH|hh|mm|ss|[MdHhmsa]|(?:(?!yyyy|yy|MM|dd|HH|hh|mm|ss|[MdHhmsa]).)+)";
...
//        StringBuffer result = new StringBuffer();
        ...
        while (matcher.find()) {
            ...
            switch (match) {
                case "yyyy":
                    results.add("TO_CHAR({0}, 'YYYY')");
//                    matcher.appendReplacement(result, "YYYY");
                    break;
                case "yy":
                    results.add("TO_CHAR({0}, 'YY')");
//                    matcher.appendReplacement(result, "YY");
                    break;
                case "MM":
                    results.add("TO_CHAR({0}, 'MM')");
//                    matcher.appendReplacement(result, "%_a1");
                    break;
                case "M":
                    results.add("TO_CHAR({0}, 'FMMM')");
                    break;
                case "dd":
                    results.add("TO_CHAR({0}, 'DD')");
//                    matcher.appendReplacement(result, "%_a2");
                    break;
                case "d":
                    results.add("TO_CHAR({0}, 'FMDD')");
                    break;
                case "HH":
                    results.add("TO_CHAR({0}, 'HH24')");
//                    matcher.appendReplacement(result, "%_a3");
                    break;
                case "H":
                    results.add("TO_CHAR({0}, 'FMHH24')");
                    break;
                case "hh":
                    results.add("TO_CHAR({0}, 'HH')");
                    break;
                case "h":
                    results.add("TO_CHAR({0}, 'FMHH')");
                    break;
                case "mm":
                    results.add("TO_CHAR({0}, 'MI')");
//                    matcher.appendReplacement(result, "%_a4");
                    break;
                case "m":
                    results.add("TO_CHAR({0}, 'FMMI')");
                    break;
                case "ss":
                    results.add("TO_CHAR({0}, 'SS')");
                    break;
                case "s":
                    results.add("TO_CHAR({0}, 'FMSS')");
//                    matcher.appendReplacement(result, "SS");
                    break;
                case "a":
                    results.add("TO_CHAR({0}, 'AM')");
