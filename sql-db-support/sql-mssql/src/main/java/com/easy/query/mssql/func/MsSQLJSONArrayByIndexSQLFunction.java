 ...
 * @author xuejiaming
 */
public class MsSQLJSONArrayByIndexSQLFunction extends AbstractExpressionSQLFunction {

    private final List<ColumnExpression> columnExpressions;

    public MsSQLJSONArrayByIndexSQLFunction(List<ColumnExpression> concatExpressions) {
        if (EasyCollectionUtil.isEmpty(concatExpressions)) {
            throw new IllegalArgumentException("MsSQLJSONArrayByIndexSQLFunction columns empty");
        }
    ...
    public String sqlSegment(TableAvailable defaultTable) {
        if(columnExpressions.size()!=2){
            throw new IllegalArgumentException("MsSQLJSONArrayByIndexSQLFunction columns size != 2");
        }
