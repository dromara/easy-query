...
//                    Assert.assertEquals("1(Integer)", EasySQLUtil.sqlParameterToString(jdbcExecuteAfterArg.getBeforeArg().getSqlParameters().get(0)));
        }
        {
            ...
            Assert.assertEquals("SELECT t2.`id` AS `id`,t2.`parent_id` AS `parent_id`,t2.`content` AS `content`,t2.`username` AS `username`,t2.`time` AS `time` FROM (SELECT t1.`id` AS `id`,t1.`parent_id` AS `parent_id`,t1.`content` AS `content`,t1.`username` AS `username`,t1.`time` AS `time` FROM (SELECT t.`id` AS `id`,t.`parent_id` AS `parent_id`,t.`content` AS `content`,t.`username` AS `username`,t.`time` AS `time`,(ROW_NUMBER() OVER (PARTITION BY t.`parent_id`)) AS `__row__` FROM `comment` t WHERE t.`parent_id` IN (?,?,?,?,?)) t1 WHERE t1.`__row__` >= ? AND t1.`__row__` <= ?) t2", jdbcExecuteAfterArg.getBeforeArg().getSql());
