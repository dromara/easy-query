...
public class NumberFunctionsTest {
    ...
    public void subQueryMySQL() {
        ...
        Assert.assertNotNull(listenerContext.getJdbcExecuteAfterArg());
        JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArg();
        Assert.assertEquals("SELECT IFNULL(t2.`__any7__`,?) AS `value1` FROM `t_sys_user` t LEFT JOIN (SELECT t1.`uid` AS `uid`,COUNT(t1.`id`) AS `__count2__`,(COUNT(*) > 0) AS `__any3__`,(COUNT((CASE WHEN t1.`uid` IS NULL THEN ? ELSE NULL END)) > 0) AS `__any4__`,(COUNT(*) <= 0) AS `__none5__`,(COUNT((CASE WHEN t1.`uid` IS NULL THEN ? ELSE NULL END)) <= 0) AS `__none6__`,(COUNT((CASE WHEN t1.`id` <> ? THEN ? ELSE NULL END)) > 0) AS `__any7__` FROM `t_bank_card` t1 GROUP BY t1.`uid`) t2 ON t2.`uid` = t.`id` WHERE IFNULL(t2.`__count2__`,0) > ? AND IFNULL(t2.`__any3__`,?) = ? AND IFNULL(t2.`__any4__`,?) = ? AND IFNULL(t2.`__any4__`,?) = ? AND IFNULL(t2.`__none5__`,?) = ? AND IFNULL(t2.`__none6__`,?) = ? AND IFNULL(t2.`__none6__`,?) = ?", jdbcExecuteAfterArg.getBeforeArg().getSql());
