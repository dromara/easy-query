...
//
    ...
    public void testBlog1() {
        ...
        Assert.assertNotNull(listenerContext.getJdbcExecuteAfterArg());
        JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArg();
        Assert.assertEquals("SELECT t.`id`,t.`title`,t.`topic`,t.`url`,t.`star`,t.`publish_time`,t.`score`,t.`status`,(ROW_NUMBER() OVER (PARTITION BY t.`topic` ORDER BY t.`score` DESC)) AS `__partition__column1` FROM `t_blog` t WHERE t.`publish_time` > ?", jdbcExecuteAfterArg.getBeforeArg().getSql());
    ...
    public void testBlog2() {
        ...
        Assert.assertNotNull(listenerContext.getJdbcExecuteAfterArg());
        JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArg();
        Assert.assertEquals("SELECT t1.`id`,t1.`title`,t1.`topic`,t1.`url`,t1.`star`,t1.`publish_time`,t1.`score`,t1.`status` FROM (SELECT t.`id`,t.`title`,t.`topic`,t.`url`,t.`star`,t.`publish_time`,t.`score`,t.`status`,(ROW_NUMBER() OVER (PARTITION BY t.`topic` ORDER BY IFNULL(t.`score`,?) DESC)) AS `__partition__column1`,(ROW_NUMBER() OVER (PARTITION BY t.`topic` ORDER BY IFNULL(t.`star`,?) ASC)) AS `__partition__column2` FROM `t_blog` t WHERE t.`publish_time` > ?) t1 WHERE t1.`__partition__column1` <= ? AND t1.`__partition__column2` <= ?", jdbcExecuteAfterArg.getBeforeArg().getSql());
    ...
    public void testBlog3() {
        ...
        Assert.assertNotNull(listenerContext.getJdbcExecuteAfterArg());
        JdbcExecuteAfterArg jdbcExecuteAfterArg = listenerContext.getJdbcExecuteAfterArg();
        Assert.assertEquals("SELECT t1.`id`,t1.`title`,t1.`topic`,t1.`url`,t1.`star`,t1.`publish_time`,t1.`score`,t1.`status`,t1.`__partition__column1` AS `__partition__column1`,t1.`__partition__column2` AS `__partition__column2` FROM (SELECT t.`id`,t.`title`,t.`topic`,t.`url`,t.`star`,t.`publish_time`,t.`score`,t.`status`,(ROW_NUMBER() OVER (PARTITION BY t.`topic` ORDER BY IFNULL(t.`score`,?) DESC)) AS `__partition__column1`,(ROW_NUMBER() OVER (PARTITION BY t.`topic` ORDER BY IFNULL(t.`star`,?) ASC)) AS `__partition__column2` FROM `t_blog` t WHERE t.`publish_time` > ?) t1 WHERE t1.`__partition__column1` <= ? AND t1.`__partition__column2` <= ?", jdbcExecuteAfterArg.getBeforeArg().getSql());
        Assert.assertEquals("0(BigDecimal),0(Integer),2024-01-01T00:00(LocalDateTime),3(Long),5(Long)", EasySQLUtil.sqlParameterToString(jdbcExecuteAfterArg.getBeforeArg().getSqlParameters().get(0)));
        listenerContextManager.clear();
    }
    @Test
